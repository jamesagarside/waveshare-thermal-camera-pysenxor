...
# =======================
MI48_SPI_MODE = 0b00
# cannot be handled with /dev/spidev-x.y and python on RPi 3B+; must work with default 8
...
# delay between asserting/deasserting CS_N and initiating/stopping clock/data
...
while True:
    ...
    img8u = cv.normalize(img.astype('uint8'), None, 255, 0,
                         ...
                         dtype=cv.CV_8U)
    img8u = cv_filter(img8u, parameters={
                      'blur_ks': 3}, use_median=False, use_bilat=True, use_nlm=False)
    if GUI:
        cv_display(img8u)
